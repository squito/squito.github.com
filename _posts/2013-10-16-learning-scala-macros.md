...
* The [academic paper on quasiquotes](http://infoscience.epfl.ch/record/185242/files/QuasiquotesForScala.pdf) was quite useful; it actually has some nice simple examples, you only need to tread the first 4 pages.  It helped me learn about both Quasiquotes, and ASTs in general
* Probably the most useful thing I learned was to **not** rely on `showRaw` as the only way to learn ASTs.  Its great because its so easy to use, but sometimes the tree it shows you is not the tree you want.  Sometimes, if you copy those trees verbatim, and you just get some obscure compiler error.
...
Now I was ready to start making my macro.  To start slowly, I wanted to see if my macro could add definitions to a class, where I just hardcoded the missing methods.  Specifically, I wanted this to work:
...
Similarly, after playing with the AST for defining classes, I discovered where a list of `DefDef`s go, so I could just add to that list:
...
Everything seemed to be working pretty well, so I put together [a few unit tests](TODO) to confirm it worked as I expected.  The tests confirmed that I could add defs to my classes, make them extend a trait, and not lose any of their existing definitions.
...
First, to get quasiquotes working in the repl, add one more import for implicit conversions:  
...
both of these are hinted at in the docs on quasiquotes and the [linked gist](https://gist.github.com/anonymous/7ab617d054f28d68901b).  Hopefully these two rules, plus the example code make it more clear.
...
The extra `.tree` and pattern match are necesary because `reify` wraps the defs into an `Expr(Block(...))`.  Simple once you know about it, but really confusing before you know what to look for.  By the time I figured this out, I already learned quasiquotes, so I just stuck with them.  But its your choice which version you prefer.
